---
meta:
    title: 'WordPress Landing Page for the Hirefast HR Tool - Webo'
    description: 'A WordPress landing page designed to drive sales and generate leads for the Hirefast HR tool. Check out how we built it!'

introduction.title: 'Hirefast - A WordPress landing page that increases the number of leads'
introduction.description: 'A WordPress landing page designed to drive sales and generate leads for the Hirefast HR tool. Check out how we built it!'
type:
    - Websites
    - WordPress Programming
technologies:
image.url: '/img/case-studies/hirefast_tile_bg.webp'
logo.url: '/img/logos/hirefast_2.svg'
tile_hidden: false
public: true
draft: false
tile_order: 6
hreflangs:
    EN: '/case-studies/hirefast/'
    PL: '/realizacje/hirefast/'
---

::page-section-md
---
class: mb-10 tablet:mb-20 mt-[100px] tablet:mt-[200px]
---
:::effect-appear-md
::::block-header-case-md
---
class: mx-default
logoUrl: '/img/logos/hirefast-black.svg'
link: 'https://hirefast.pl/'
---

#title
# Hirefast - A WordPress landing page that increases the number of leads

#description
**Hirefast** is a platform that helps companies hire employees and manage the recruitment process end-to-end. Our task was to **design and implement a landing page** aimed at supporting the sales process and **effectively generating qualified leads**.


#buttontext
See online

#tags
- Webpage
- UX/UI Design
- Wordpress programming

::::
:::
::

::page-section-md
---
class: mb-[50px] tablet:mb-[100px]
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img1.webp"}
::::
:::
::


::page-section-md
---
class: _static-content mb-[50px] tablet:mb-[100px]
---

:::effect-appear-md
---
class: _mx-default
---
::::block-columns-1-1-md
---
reverseMobile: true
class: items-center [&_div.right]:self-start
---

#left
::::card-icon-md
---
class: tablet:pr-2.5 [&_div_div.group]:p-10 [&_div_div]:bg-primary [&_div_div]:!text-gray-darker [&_h4]:!mb-4 [&_h4]:tablet:text-[35px]
icon: '/img/icons/info.svg'
---
#title
#### Project in a nutshell

#content
:::::block-list-ul-md
---
decoration: false
---
::::::list-item-ul-active-md
---
iconUrl: '/img/icons/list-1.svg'
iconBig: false
animated: false
class: icon [&_div_div]:!pb-1 [&_div_div_div]:!min-h-fit [&_div_div.flex]:lg:flex-row [&_div_div.flex]:gap-7 [&_div_div_picture]:!w-[60px] [&_div.title_p]:!text-xl [&_div.title]:!mb-2 [&_div_div_div_div_p]:!tracking-normal

---

#title
Development environment setup

#description
We set up a development environment along with a project repository on GitHub, which ensured full control over the codebase and improved team collaboration.

::::::
::::::list-item-ul-active-md
---
iconUrl: '/img/icons/list-2.svg'
iconBig: false
animated: false
class: icon [&_div_div]:!pb-1 [&_div_div_div]:!min-h-fit [&_div_div.flex]:lg:flex-row [&_div_div.flex]:gap-7 [&_div_div_picture]:!w-[60px] [&_div.title_p]:!text-xl [&_div.title]:!mb-2 [&_div_div_div_div_p]:!tracking-normal
---

#title
WordPress development

#description
We implemented the landing page using WordPress, creating **flexible and reusable components** with ACF Pro. This enabled **easy content management** and further **scalability of the website without the need for code-level changes.**


::::::
::::::list-item-ul-active-md
---
iconUrl: '/img/icons/list-3.svg'
iconBig: false
animated: false
class: icon [&_div_div]:!pb-1 [&_div_div_div]:!min-h-fit [&_div_div.flex]:lg:flex-row [&_div_div.flex]:gap-7 [&_div_div_picture]:!w-[60px] [&_div.title_p]:!text-xl [&_div.title]:!mb-1 [&_div_div_div_div_p]:!tracking-normal
---

#title
Testing and launch

#description
We conducted **cross-device testing of the website**, with particular focus on the lead generation flow. We then launched the landing page on the production domain and **configured analytics and marketing tools** to support sales activities.

::::::
:::::
::::

#right

::::card-icon-md
---
class: tablet:pr-2.5 [&_div_div.group]:p-10 [&_div_div]:bg-white [&_div_div]:!text-gray-darker [&_h4]:!mb-4 [&_h4]:tablet:text-[35px]
icon: '/img/icons/double_check.svg'
---
#title
#### Results and ongoing support

#content
:::::block-list-ul-md
---
decoration: false
---
::::::list-item-ul-active-md
---

iconBig: false
animated: false
class: icon [&_div_div_div]:!min-h-fit 
---

#description
The result of our work is a **modern WordPress landing page** supporting the sales of the Hirefast tool and **effective lead generation.** The website was designed to ensure a clear product presentation, intuitive navigation, and maximum user conversion across all devices.
<br>
<br>
The high quality of implementation, code optimization, and a well-structured page architecture improve user experience and conversion efficiency, directly supporting sales activities.

::::::
:::::


::::
:::
::



::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img2.webp"}
::::
:::
::

::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img3.webp"}
::::
:::
::

::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img4.webp"}
::::
:::
::

::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img5.webp"}
::::
:::
::

::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img6.webp"}
::::
:::
::

::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img7.webp"}
::::
:::
::

::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img8.webp"}
::::
:::
::

::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img9.webp"}
::::
:::
::

::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img10.webp"}
::::
:::
::

::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img11.webp"}
::::
:::
::

::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img12.webp"}
::::
:::
::

::page-section-md
---
class: mb-5 tablet:mb-[100px]
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-hirefast-img13.webp"}
::::
:::
::



::page-section-md
---
class: mb-[50px] tablet:mb-[100px]
---
:::effect-appear-md
::::block-cta-md
---
button.link: https://webo.agency/contact/
button.title: 'Get in touch'
---

#title
#### Need experts to implement a *website?*

#description
Schedule a free consultation, let's talk!

::::
:::
::

::page-section-md
---
id: case-studies
class: mb-[80px] tablet:mb-[130px]
---
:::effect-appear-md
::::block-header-md
---
class: mb-10 tablet:mb-20
---

#subtitle
### *2/* Our recent projects

#title
## See our other *website projects*

::::
:::

:::block-cases-md
---
contentFolder: case-studies
moreBtn.url: 'https://webo.agency/case-studies'
moreBtn.linktitle: See all projects
selectedSlugs:
    - isw
    - mutiny-group
---

:::
::

::page-section-md
---
class: mb-20 tablet:mb-[130px]
---
:::effect-appear-md
::::block-header-md
---
center: true
class: mb-[50px] tablet:mb-[100px]
---

#subtitle
### *3/* Our clients

#title
## Meet the companies that *trusted us*

::::
:::
:::block-brands-before-footer-md
:::
::
